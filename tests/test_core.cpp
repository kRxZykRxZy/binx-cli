...
int main(){
 ...
 auto t1=std::filesystem::temp_directory_path()/"binx-diff-a.bin";auto t2=std::filesystem::temp_directory_path()/"binx-diff-b.bin";{std::ofstream fa(t1,std::ios::binary),fb(t2,std::ios::binary);fa<<"abcdef";fb<<"abXdefg";}auto ba=BinaryFile::open(t1);auto bb=BinaryFile::open(t2);assert(ba&&bb);auto dr=compare_binaries(ba.value(),bb.value());assert(dr&&dr.value().changed_bytes==1&&dr.value().added_bytes==1&&dr.value().removed_bytes==0&&dr.value().hunks.size()==1);auto sr=analyze_size(ba.value());assert(sr&&sr.value().file_size==6);std::filesystem::remove(t1);std::filesystem::remove(t2);
 return 0;
}