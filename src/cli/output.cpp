...
std::string join_json(const std::vector<std::string>&v){std::ostringstream o;o<<"[";for(std::size_t i=0;i<v.size();++i){if(i)o<<",";o<<"\""<<json_escape(v[i])<<"\"";}o<<"]";return o.str();}
void diagnostic_json(std::ostringstream&o,const Diagnostic&d){o<<"{\"severity\":\""<<diagnostic_severity_name(d.severity)<<"\",\"code\":\""<<json_escape(d.code)<<"\",\"component\":\""<<json_escape(d.component)<<"\"";if(d.file_offset)o<<",\"file_offset\":\""<<hx(*d.file_offset)<<"\"";if(d.rva)o<<",\"rva\":\""<<hx(*d.rva)<<"\"";o<<",\"message\":\""<<json_escape(d.message)<<"\"}";}
std::string generic_json(const BinaryFile&f){const auto&m=f.metadata();std::ostringstream o;o<<"{\n  \"schema_version\":1,\n  \"file\":{\"name\":\""<<json_escape(f.path().filename().string())<<"\",\"size\":"<<m.file_size<<"},\n  \"format\":\""<<format_name(m.format)<<"\",\n  \"architecture\":\""<<architecture_name(m.architecture)<<"\",\n  \"endianness\":\""<<endianness_name(m.endianness)<<"\",\n  \"platform\":\""<<json_escape(m.platform)<<"\"";if(m.entry_point)o<<",\n  \"entry_point\":\""<<hx(*m.entry_point)<<"\"";if(m.image_base)o<<",\n  \"image_base\":\""<<hx(*m.image_base)<<"\"";o<<"\n}\n";return o.str();}
}
std::string format_help(){
 ...
 "  symbols     List ELF/PE/debug symbols\n"
 "  dynamic     Show ELF dynamic table\n"
 "  notes       Show ELF notes\n"
 "  strings     Extract ASCII/UTF strings\n"
 "  hexdump     Display a bounded hexadecimal view\n"
 "  search      Search text or wildcard hex bytes\n"
 "  regions     Classify byte regions\n"
 ...
 "  disasm      Disassemble supported x86 code\n"
 "  debug       Inspect DWARF/PDB metadata\n"
 "  crash       Analyze Windows minidumps/ELF cores\n"
 "  report      Generate a unified analysis report\n"
 "  analyze     Alias for report\n"
 ...
 "  --encoding VALUE       ascii, utf8, utf16le, utf16be, all\n"
 "  --min-length N         Minimum extracted string length\n"
 "  --max-results N        Maximum result count\n"
 "  --offset N             Starting file offset (decimal or 0x hex)\n"
 "  --length N             Number of bytes to read\n"
 "  --width N              Hexdump width (1..256)\n"
 "  --pattern/--hex VALUE  Wildcard hex pattern\n"
 "  --text VALUE           Text search pattern\n"
 "  --region-min N         Minimum region size\n"
 "  --search-path PATH     Dependency search directory (repeatable)\n"
 "  --recursive            Resolve dependencies recursively\n"
 "  --depth N              Dependency depth (0..64)\n"
 "  --max-nodes N          Dependency node limit\n"
 "  --dot                  Emit Graphviz DOT\n"
 "  --from-entry           Start disassembly at the entry point\n"
 "  --count N              Maximum instructions\n"
 "  --max-bytes N          Maximum disassembly bytes\n"
 "  --syntax VALUE         intel or att\n"
...
std::string format_pe_command(const PEImage&im,const std::string&cmd,bool json,bool detailed){
    if(json){
        ...
        for(std::size_t i=0;i<im.sections.size();++i){if(i)o<<",";const auto&s=im.sections[i];o<<"{\"index\":"<<s.index<<",\"name\":\""<<json_escape(s.name)<<"\",\"virtual_size\":"<<s.virtual_size<<",\"virtual_address\":\""<<hx(s.virtual_address)<<"\",\"raw_size\":"<<s.raw_size<<",\"raw_pointer\":\""<<hx(s.raw_pointer)<<"\",\"characteristics\":"<<s.characteristics<<",\"flags\":\""<<flags(s)<<"\"}";}
        o<<"],\n  \"imports\":[";if(im.imports){for(std::size_t i=0;i<im.imports->modules.size();++i){if(i)o<<",";const auto&m=im.imports->modules[i];o<<"{\"dll\":\""<<json_escape(m.dll_name)<<"\",\"lookup_rva\":\""<<hx(m.lookup_rva)<<"\",\"iat_rva\":\""<<hx(m.iat_rva)<<"\",\"symbols\":[";for(std::size_t j=0;j<m.symbols.size();++j){if(j)o<<",";const auto&s=m.symbols[j];o<<"{\"name\":"<<(s.name?"\""+json_escape(*s.name)+"\"":"null")<<",\"ordinal\":"<<(s.ordinal?std::to_string(*s.ordinal):"null")<<",\"hint\":"<<(s.hint?std::to_string(*s.hint):"null")<<",\"lookup_rva\":\""<<hx(s.lookup_rva)<<"\",\"iat_rva\":\""<<hx(s.iat_rva)<<"\"}";}o<<"]}";}}o<<"],\n  \"exports\":[";if(im.exports){for(std::size_t i=0;i<im.exports->functions.size();++i){if(i)o<<",";const auto&f=im.exports->functions[i];o<<"{\"ordinal\":"<<f.ordinal<<",\"rva\":\""<<hx(f.rva)<<"\",\"name\":"<<(f.name?"\""+json_escape(*f.name)+"\"":"null")<<",\"forwarder\":"<<(f.forwarder?"\""+json_escape(*f.forwarder)+"\"":"null")<<"}";}}o<<"],\n  \"relocations\":[";if(im.relocations){for(std::size_t i=0;i<im.relocations->blocks.size();++i){if(i)o<<",";const auto&b=im.relocations->blocks[i];o<<"{\"page_rva\":\""<<hx(b.page_rva)<<"\",\"block_size\":"<<b.block_size<<",\"entries\":"<<b.entries.size()<<"}";}}o<<"],\n  \"resources\":[";if(im.resources){for(std::size_t i=0;i<im.resources->items.size();++i){if(i)o<<",";const auto&r=im.resources->items[i];o<<"{\"path\":[";for(std::size_t j=0;j<r.path.size();++j){if(j)o<<",";o<<"\""<<json_escape(r.path[j])<<"\"";}o<<"],\"language\":"<<r.language<<",\"data_rva\":\""<<hx(r.data_rva)<<"\",\"data_size\":"<<r.data_size<<",\"code_page\":"<<r.code_page<<",\"file_offset\":"<<(r.file_offset?"\""+hx(*r.file_offset)+"\"":"null")<<"}";}}o<<"],\n  \"tls\":{\"status\":\""<<pe_status_name(im.tls_status)<<"\",\"callbacks\":"<<(im.tls?im.tls->callbacks.size():0)<<",\"address_of_callbacks\":\""<<(im.tls?hx(im.tls->address_of_callbacks_va):"")<<"\"},\n  \"debug\":{\"status\":\""<<pe_status_name(im.debug_status)<<"\",\"entries\":"<<im.debug_entries.size()<<"},\n  \"statuses\":{\"headers\":\""<<pe_status_name(im.header_status)<<"\",\"sections\":\""<<pe_status_name(im.section_status)<<"\",\"imports\":\""<<pe_status_name(im.import_status)<<"\",\"exports\":\""<<pe_status_name(im.export_status)<<"\",\"relocations\":\""<<pe_status_name(im.relocation_status)<<"\",\"resources\":\""<<pe_status_name(im.resource_status)<<"\",\"tls\":\""<<pe_status_name(im.tls_status)<<"\",\"debug\":\""<<pe_status_name(im.debug_status)<<"\"},\n  \"diagnostics\":[";for(std::size_t i=0;i<im.diagnostics.size();++i){if(i)o<<",";diagnostic_json(o,im.diagnostics[i]);}o<<"]\n}\n";return o.str();}
...
std::string format_elf_command(const ELFImage&i,const std::string&cmd,bool json,bool detailed){
 auto hx=[](std::uint64_t v){std::ostringstream s;s<<"0x"<<std::hex<<std::uppercase<<v;return s.str();};
 auto esc2=[](const std::string&s){std::string o;for(char c:s){if(c=='"')o+="\\\"";else if(c=='\\')o+="\\\\";
 else o+=c;}return o;};
 if(json){
  std::ostringstream o;o<<"{\n  \"schema_version\":3,\n  \"format\":\""<<(i.elf_class==ELFClass::ELF64?"ELF64":"ELF32")<<"\",\n";
  o<<"  \"header\":{\"type\":\""<<elf_type_name(i.type)<<"\",\"machine\":\""<<elf_machine_name(i.machine)<<"\",\"entry\":\""<<hx(i.entry)<<"\",\"program_headers\":"<<i.phnum<<",\"section_headers\":"<<i.shnum<<",\"interpreter\":"<<(i.interpreter?"\""+esc2(*i.interpreter)+"\"":"null")<<"},\n";
  o<<"  \"sections\":[";for(std::size_t n=0;n<i.sections.size();n++){if(n)o<<",";auto&s=i.sections[n];o<<"{\"index\":"<<s.index<<",\"name\":\""<<esc2(s.name)<<"\",\"type\":\""<<elf_section_type_name(s.type)<<"\",\"flags\":\""<<elf_section_flags(s.flags)<<"\",\"address\":\""<<hx(s.address)<<"\",\"offset\":\""<<hx(s.offset)<<"\",\"size\":"<<s.size<<",\"link\":"<<s.link<<",\"info\":"<<s.info<<"}";}o<<"],\n  \"segments\":[";for(std::size_t n=0;n<i.segments.size();n++){if(n)o<<",";auto&s=i.segments[n];o<<"{\"index\":"<<s.index<<",\"type\":\""<<elf_segment_type_name(s.type)<<"\",\"flags\":\""<<elf_segment_flags(s.flags)<<"\",\"offset\":\""<<hx(s.offset)<<"\",\"vaddr\":\""<<hx(s.vaddr)<<"\",\"file_size\":"<<s.file_size<<",\"memory_size\":"<<s.memory_size<<"}";}o<<"],\n  \"symbols\":"<<i.symbols.size()<<",\"dynamic_entries\":"<<i.dynamic.size()<<",\"notes\":"<<i.notes.size()<<",\"relocations\":"<<i.relocations.size()<<",\n  \"statuses\":{\"header\":\""<<pe_status_name(i.header_status)<<"\",\"sections\":\""<<pe_status_name(i.section_status)<<"\",\"segments\":\""<<pe_status_name(i.segment_status)<<"\",\"symbols\":\""<<pe_status_name(i.symbol_status)<<"\",\"dynamic\":\""<<pe_status_name(i.dynamic_status)<<"\",\"notes\":\""<<pe_status_name(i.note_status)<<"\",\"relocations\":\""<<pe_status_name(i.relocation_status)<<"\"},\n  \"diagnostics\":[";for(std::size_t n=0;n<i.diagnostics.size();n++){if(n)o<<",";diagnostic_json(o,i.diagnostics[n]);}o<<"]\n}\n";return o.str();
 }
 std::ostringstream o;o<<"BinX ELF Inspector v"<<BINX_VERSION<<"\n\nELF HEADER\n  Class:           "<<(i.elf_class==ELFClass::ELF64?"ELF64":"ELF32")<<"\n  Endianness:      "<<endianness_name(i.endianness)<<"\n  Type:            "<<elf_type_name(i.type)<<" ("<<i.type<<")\n  Machine:         "<<elf_machine_name(i.machine)<<" ("<<i.machine<<")\n  Entry point:     "<<hx(i.entry)<<"\n  Program headers: "<<i.phnum<<"\n  Section headers: "<<i.shnum<<"\n";if(i.interpreter)o<<"  Interpreter:     "<<*i.interpreter<<"\n";o<<"\n";
 if(cmd=="sections"||cmd=="inspect"){o<<"SECTIONS\n  #   Name                 Type       Address        Offset         Size        Flags\n";for(auto&s:i.sections)o<<"  "<<std::setw(3)<<s.index<<" "<<std::left<<std::setw(20)<<s.name<<std::right<<" "<<std::setw(10)<<elf_section_type_name(s.type)<<" "<<std::setw(14)<<hx(s.address)<<" "<<std::setw(14)<<hx(s.offset)<<" "<<std::setw(10)<<s.size<<" "<<elf_section_flags(s.flags)<<"\n";o<<"\n";}
 if(cmd=="segments"||cmd=="inspect"){o<<"PROGRAM SEGMENTS\n  #   Type       Flags  Offset         VAddr          FileSize    MemSize\n";for(auto&s:i.segments)o<<"  "<<std::setw(3)<<s.index<<" "<<std::left<<std::setw(10)<<elf_segment_type_name(s.type)<<std::right<<" "<<std::setw(5)<<elf_segment_flags(s.flags)<<" "<<std::setw(14)<<hx(s.offset)<<" "<<std::setw(14)<<hx(s.vaddr)<<" "<<std::setw(10)<<s.file_size<<" "<<s.memory_size<<"\n";o<<"\n";}
 if(cmd=="symbols"||cmd=="inspect"){o<<"SYMBOLS\n";for(auto&s:i.symbols)o<<"  "<<std::setw(18)<<hx(s.value)<<" "<<std::left<<std::setw(8)<<elf_symbol_bind_name(s.bind)<<" "<<std::setw(8)<<elf_symbol_type_name(s.type)<<" "<<s.name<<std::right<<"\n";o<<"\n";}
 if(cmd=="dynamic"||cmd=="inspect"){o<<"DYNAMIC\n";for(auto&s:i.dynamic)o<<"  "<<std::setw(16)<<s.tag<<" "<<std::left<<std::setw(10)<<elf_dynamic_tag_name(s.tag)<<std::right<<" "<<hx(s.value)<<"\n";o<<"\n";}
 if(cmd=="relocations"||cmd=="inspect"){o<<"RELOCATIONS\n";for(auto&s:i.relocations)o<<"  "<<hx(s.offset)<<" type="<<s.type<<" symbol="<<s.symbol<<" "<<s.symbol_name<<(s.has_addend?" addend="+std::to_string(s.addend):"")<<"\n";o<<"\n";}
 if(cmd=="notes"||cmd=="inspect"){o<<"NOTES\n";for(auto&s:i.notes)o<<"  "<<s.name<<" type="<<s.type<<" desc_size="<<s.desc.size()<<"\n";o<<"\n";}
 if(cmd=="inspect"||detailed){o<<"VALIDATION\n";for(auto&d:i.diagnostics)o<<"  ["<<diagnostic_severity_name(d.severity)<<"] "<<d.code<<": "<<d.message<<"\n";}
 return o.str();
}
