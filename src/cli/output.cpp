...
void print_info(const BinaryFile& f,bool json){
 ...
 if(json){
  ...
  std::cout<<"  \"platform\": \""<<esc(m.platform)<<"\",\n";
...
void print_hashes(const Hashes& h,bool json){
 if(json) std::cout<<"{\"md5\":\""<<h.md5<<"\",\"sha1\":\""<<h.sha1<<"\",\"sha256\":\""<<h.sha256<<"\"}\n";
