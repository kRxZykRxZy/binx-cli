...
bool parse_hex_pattern(const std::string&in,std::vector<int>&out){out.clear();auto hex=[](char c)->int{if(c>='0'&&c<='9')return c-'0';if(c>='a'&&c<='f')return c-'a'+10;if(c>='A'&&c<='F')return c-'A'+10;return -1;};std::size_t i=0;while(i<in.size()){char c=in[i];if(c==' '||c=='\t'||c=='\r'||c=='\n'){++i;continue;}if(c=='?'){++i;if(i<in.size()&&in[i]=='?')++i;out.push_back(-1);continue;}int a=hex(c);if(a<0)return false;++i;if(i>=in.size())return false;int b=hex(in[i]);if(b<0)return false;++i;out.push_back(a*16+b);}return !out.empty();}
