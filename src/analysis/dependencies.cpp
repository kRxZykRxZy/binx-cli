...
std::optional<std::filesystem::path> resolve(const std::filesystem::path&parent,const std::string&name,const std::vector<std::filesystem::path>&search){
 std::string expanded=name;const std::string a="@loader_path/",b="@executable_path/",o1="$ORIGIN/",o2="$ORIGIN\\";
 if(expanded.rfind(a,0)==0)expanded=(parent/expanded.substr(a.size())).string();else if(expanded.rfind(b,0)==0)expanded=(parent/expanded.substr(b.size())).string();else if(expanded.rfind(o1,0)==0)expanded=(parent/expanded.substr(o1.size())).string();else if(expanded.rfind(o2,0)==0)expanded=(parent/expanded.substr(o2.size())).string();
