...
bool parse(int argc,char**argv,Options&o){
    ...
    if(o.command=="help"||o.command=="version"||o.command=="--help"||o.command=="--version")return true;
...
int main(int argc,char**argv){
    ...
    if(o.command.empty()||o.command=="help"||o.command=="--help"){std::cout<<format_help();return 0;}
    if(o.command=="version"||o.command=="--version"){std::cout<<format_version();return 0;}
    if(o.command!="info"&&o.command!="inspect"&&o.command!="hash"&&!pe_command(o.command)){err("unknown command '"+o.command+"'");return static_cast<int>(ErrorCode::InvalidArguments);}
    ...
    if(o.verbose)std::cerr<<"[binx] PE parser: "<<pe_status_name(image.header_status)<<", sections="<<image.sections.size()<<", diagnostics="<<image.diagnostics.size()<<"\n";
