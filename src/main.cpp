...
namespace {
struct Options{std::string command;std::filesystem::path input;std::filesystem::path output;bool has_output=false;bool json=false;bool quiet=false;bool verbose=false;std::string encoding="ascii";std::string pattern;std::string text;std::uint64_t offset=0;std::uint64_t length=0;std::size_t width=16;std::size_t min_length=4;std::size_t max_results=0;std::size_t region_min=16;std::vector<std::filesystem::path> search_paths;std::size_t depth=8;std::size_t max_nodes=256;bool dot=false;bool recursive=false;std::size_t count=100;std::size_t max_bytes=0;bool from_entry=false;DisassemblySyntax syntax=DisassemblySyntax::Intel;};
...
bool dependency_command(const std::string&c){return c=="deps"||c=="dependencies"||c=="graph";} bool disasm_command(const std::string&c){return c=="disasm"||c=="disassemble";} bool symbols_command(const std::string&c){return c=="symbols"||c=="sym";} bool debug_command(const std::string&c){return c=="debug"||c=="debug-info";}
}
int main(int argc,char**argv){
 ...
 if(o.command!="info"&&o.command!="inspect"&&o.command!="hash"&&!analysis_command(o.command)&&!dependency_command(o.command)&&!disasm_command(o.command)&&!symbols_command(o.command)&&!debug_command(o.command)&&!pe_command(o.command)&&!elf_command(o.command)){err("unknown command '"+o.command+"'");return static_cast<int>(ErrorCode::InvalidArguments);}
