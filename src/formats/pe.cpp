...
void parse_sections(std::span<const std::byte> data,PEImage& image,std::size_t offset){
    ...
    for(std::size_t i=0;i<image.sections.size();++i)for(std::size_t j=i+1;j<image.sections.size();++j){
        ...
        const auto ar=std::uint64_t(a.raw_pointer)+a.raw_size; const auto br=std::uint64_t(b.raw_pointer)+b.raw_size;
...
Result<PEImage> parse_pe(std::span<const std::byte> data){
    if(data.size()<2||std::to_integer<unsigned char>(data[0])!='M'||std::to_integer<unsigned char>(data[1])!='Z')return Error{ErrorCode::UnsupportedFormat,"input is not a PE image"};
    PEImage image;
